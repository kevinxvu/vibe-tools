...
func CORS(cfg *Config) echo.MiddlewareFunc {
	...
	return middleware.CORSWithConfig(middleware.CORSConfig{
		...
		AllowHeaders:     []string{"Origin", "Content-Type", "Accept", "Authorization", "X-App-Id"},
...
// VerifyHeader validates that the request contains the expected header key/value pair.
// If headerKey or headerValue is empty, the check is skipped.
func VerifyHeader(headerKey, headerValue string) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if c.Request().Header.Get(headerKey) != headerValue {
				return echo.NewHTTPError(http.StatusUnauthorized, fmt.Sprintf("invalid header value for %s", headerKey))
			}
			return next(c)
		}
	}
